...
select distinct on (client_id) client_id,
	   first_part,
	   amount min_amount,
	   rank()over(partition by client_id order by amount ASc)
FROM all_transactions  )
...
select distinct on (client_id) client_id,
	   first_part,
	   amount max_amount,
	   rank()over(partition by client_id order by amount desc)
FROM all_transactions  )
...
select DISTINCT ON (all_transactions.client_id) all_transactions.client_id,
...
FROM all_transactions
join frequency on frequency.client = all_transactions.client_id
join num_loan_apps on num_loan_apps.client_id = all_transactions.client_id
join min_borrowed on min_borrowed.client_id = all_transactions.client_id
join max_borrowed on max_borrowed.client_id = all_transactions.client_id
GROUP BY all_transactions.client_id,
