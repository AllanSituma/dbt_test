...
all_clients as (
    SELECT lf."ClientID" client_id,
           lf."NonPerformingLoans" npl
    FROM loan_first lf
)

SELECT DISTINCT ON (ac.client_id) ac.client_id,
        ac.npl,
...
FROM all_clients ac
LEFT JOIN airtime_purchase ap ON ap.client_id = ac.client_id
LEFT JOIN bank_deposit bd ON bd.client_id = ac.client_id
LEFT JOIN bank_withdraw bw ON bw.client_id = ac.client_id
LEFT JOIN betting_receipts br ON br.client_id = ac.client_id
LEFT JOIN betting_spends bs ON bs.client_id = ac.client_id
LEFT JOIN lipa_mpesa lm ON lm.client_id = ac.client_id
LEFT JOIN loanapp_borrowing lb ON lb.client_id = ac.client_id
LEFT JOIN loanapp_repayment lr ON lr.client_id = ac.client_id
LEFT JOIN loanapps_used lu ON lu.client_id = ac.client_id
LEFT JOIN mshwari m ON m.client_id = ac.client_id
LEFT JOIN bank_payments bp ON bp.client_id = ac.client_id
LEFT JOIN sacco_savings ss ON ss.client_id = ac.client_id
LEFT JOIN sacco_withdraw sw ON sw.client_id = ac.client_id
